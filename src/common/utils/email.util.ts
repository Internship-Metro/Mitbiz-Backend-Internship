...
const transporter = nodemailer.createTransport({
  host: env.SMTP_HOST,
  port: env.SMTP_PORT,
  secure: Number(env.SMTP_PORT) === 465, // true untuk 465, false untuk 587
  requireTLS: true,
  auth: {
    user: env.SMTP_USER,
    pass: env.SMTP_PASS,
...
export async function sendVerificationEmail(
  ...
  try {
    ...
    console.log(`[Email] Email verifikasi berhasil dikirim ke ${toEmail}`);
  } catch (error) {
    console.error(`[Email Error] Gagal mengirim email ke ${toEmail}:`, error);
    // Jangan throw error, agar pendaftaran user tetap berhasil meskipun email gagal terkirim
  }
}
